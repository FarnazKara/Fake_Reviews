    ...
    def from_batch_translator_output(self, outputs):
        ...
        for b in range(batch_size):
            pred_sents = [self._build_sentence(
                src[:,b:], preds[b][n], self.trg_vocab, attns[b][n]) for n in range(self.k_best)]
            gold = trg[1:,b:].squeeze().cpu().numpy()
            input = src[:,b:].squeeze().cpu().numpy()

            input_sent = self._build_sentence(src[:,b:], input, self.src_vocab, None)
            gold_sent = self._build_sentence(src[:,b:], gold, self.trg_vocab, None)
