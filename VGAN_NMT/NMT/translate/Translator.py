...
class BatchTranslator(object):
    ...
    def _run_target(self, batch):
        ...
        if torch.cuda.is_available():
            ...
            gold_scores = torch.FloatTensor(batch.batch_size).fill_(0)
            gold_scores = gold_scores.to(torch.device("cpu"))
        ...
        for dec, trg in zip(decoder_output, trg_out.data):
            ...
            #scores = out.data.gather(1, out.data.gather(1, trg))
            scores = out.data.gather(1, trg.reshape(1,-1))
